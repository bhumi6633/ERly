...
5. Click **Apply**. Render builds and deploys. Tables are created on startup; `preDeployCommand` runs `python seed.py` so the DB is seeded with care locations on first deploy.
