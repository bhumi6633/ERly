...
function TypeChip({ type }: { type: string }) {
    return (
        <span className="inline-block px-1.5 py-px rounded text-[9px] font-bold uppercase tracking-wider bg-white/[0.05] text-white/40 border border-white/[0.07]">
            {type}
        </span>
    );
}
    ...
    return (
        <aside className="absolute left-4 top-1/2 -translate-y-1/2 z-10 w-80 max-h-[78vh] flex flex-col glass rounded-2xl overflow-hidden animate-slideInLeft">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] shrink-0">
                <div>
                    <div className="text-[9px] uppercase tracking-[0.25em] font-bold text-white/25 mb-1">
                        ERly · Triage
                    </div>
                ...
                <button
                    onClick={onClose}
                    className="p-1.5 rounded-lg hover:bg-white/[0.07] text-white/35 hover:text-white/70 transition-colors"
                >
                    <X size={14} />
                </button>
            </div>
            ...
            <div className="px-5 py-3.5 border-b border-white/[0.07] shrink-0">
                ...
                <div className="flex items-baseline gap-1.5 mt-2">
                    <span className="text-[9px] uppercase tracking-wider text-white/25 font-bold">Recommended</span>
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
                <div className="px-5 pt-4 pb-2 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-white/20 font-mono tabular-nums">{result.facilities.length}</span>
                </div>
                                ...
                                >
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex items-start gap-2 min-w-0">
                                            <span className="text-[10px] font-mono text-white/20 shrink-0 mt-0.5 w-4 text-right select-none">
                                                {idx + 1}
                                            </span>
                                        ...
                                        {totalMin != null && (
                                            <div className="shrink-0 text-right leading-none">
                                                ...
                                                <div className="text-[8px] uppercase tracking-wider text-white/25 font-bold mt-0.5">
                                                    total
                                                </div>
                                            </div>
                                        )}
                                    ...
                                    <div className="flex items-center gap-1.5 mt-2 pl-6 flex-wrap">
                                        ...
                                        {facility.distance && (
                                            <>
                                                ...
                                                <span className="text-white/35 text-[11px]">{facility.distance}</span>
                                            </>
                                        )}
